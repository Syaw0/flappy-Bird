...
let bird , background , gameover  , animationFrame , isFirstTime , menu , score , uniqIdForBarrier , barrierArr , passedBarrierObj
barrierArr = [] 
passedBarrierObj = {}
canvas.width = 420
canvas.height = 500
...
const buttonHandle = (e) => {
    if(e.offsetX  > gameover.restartBtnX && e.offsetX  < (gameover.restartBtnX + gameover.restartBtnSizeX) ){
        if(e.offsetY  > gameover.restartBtnY && e.offsetY < (gameover.restartBtnSizeY + gameover.restartBtnY)){
            cancelAnimationFrame(animationFrame);
            init();
            bird.live = true;
            uniqIdForBarrier = 0 ;
...
const animateHanlde = () => {
    ...
    barrierHanlde();
    birdHanlde();
    !isFirstTime && !gameover.status ? score.draw(Object.keys(passedBarrierObj).length) : undefined
    ...
    if(barrierArr.length < 3){
        for(let i = barrierArr.length ; i <3 ; i++){
            if(barrierArr[0] == undefined ){ //if array is empty do this
                barrierArr.push(new Barrier(ctx ,canvas.height , canvas.width , canvas.width , 0 ))
                ...
                uniqIdForBarrier ++
                barrierArr.push(new Barrier(ctx ,canvas.height ,canvas.width , distance , uniqIdForBarrier))
            }
        }
    }
}
...
const isBarrierTouchBird = (barrierNum) => {
    ...
    if(barrier.status.y2 == undefined){ //one barrrier
        if(bird.x > barrier.x && bird.x < barrier.x + barrier.barrierWidth){
            if(bird.y > barrier.status.y && bird.y < barrier.status.y + barrier.barrierHeight){
                gameover.status = true ; 
            }
        }
    }else{ //two barrier
        if(bird.x > barrier.x && bird.x < barrier.x + barrier.barrierWidth){
            if(bird.y >  barrier.status.y && bird.y < barrier.status.y + barrier.barrierHeight){
                gameover.status = true;
            }else if (bird.y > barrier.status.y2 && bird.y < barrier.status.y2 + barrier.barrierHeight){
                gameover.status = true ;
            }
        }
    }
